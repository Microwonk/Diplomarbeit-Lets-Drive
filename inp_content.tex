...
Dieser spezielle Bereich ist von besonderer Relevanz, da er unmittelbaren Einfluss auf das Gesamtergebniss der App hat. Eine gründliche Untersuchung dieses Themas wird dazu beitragen, die Wünsche der Benutzer zu definieren und darauf die App zu designen und aufzubauen.

Im Folgenden werden wir die USE-Cases, das Domain-Class-Modelling und User-Interface im Detail darstellen. Dies ermöglicht es uns, die Grundlagen für unsere weitere Arbeit zu legen.

\subsection{USE-Case-Analyse}

USE-Cases, auch Anwendungsfälle genannt, sind eine wichtige Komponente in einer Diplomarbeit. Sie beschreiben, wie ein bestimmtes System oder eine Software in der Praxis eingesetzt wird. Dabei unterscheidet man zwischen Benutzer-basierten und Ereignis-basierten USE-Cases:
\begin{itemize}
...
\newpage
USE-Cases dieses Projektes können mithilfe von Tabellen (\ref{tab:USE-Case1}), USE-Case Diagrammen (\ref{fig:USE-Case-Diagramm}) und mittles Aktivitätsdiagrammen (\ref{}) dargestellt werden. \\
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|l|X|}
		...
		Referenzen &  \\
		\hline
		kurze Beschreibung & Das Fahrtenbuch wird anhand der Rohdaten jedes mal neu erstellt. Wenn man die Statistik bzw. die Details wissen will, werden die Rohdaten mit Graphen und auf der Map dargestellt. \\
		\hline
		Akteure &  \\
		\hline
